...
// 등록되지 않은 이름, 디렉토리를 쓸 수 없는 프로필, PATH에서 실행 파일을 찾지 못한 세 상황에서
// use와 logout이 같은 이름·같은 상태에 대해 같은 종료 코드로 거부되고, stderr 한 줄이 접두사까지
// 같은 문자열이다(SPEC §5.4) — 둘 다 target.go의 같은 판정 함수를 거치기 때문이다.
func TestUseAndLogout_RejectWithSameMessageAndCode(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.label, func(t *testing.T) {
			...
			_, logoutStderr, logoutErr := runUseCLI(t, layout, tc.launcher(), nil, "logout", name)
			if logoutErr == nil {
				t.Fatalf("logout %q error = nil, want a rejection", name)
			}
			if got := cli.ExitCode(logoutErr); got != tc.wantCode {
				t.Errorf("logout ExitCode() = %d, want %d", got, tc.wantCode)
			}

			if useStderr != logoutStderr {
				t.Errorf("stderr differs:\nuse    = %q\nlogout = %q", useStderr, logoutStderr)
			}
